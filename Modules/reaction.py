...
class Reactions(BOT.Cog):
    ...
    async def hey(self, ctx, member: discord.Member = None):
        ...
        mass_all = [
            "поприветствовал всех!", "поздоровался со всеми!",
            "сказал Салам Алейкум всем!", "сказал всем ДОБРОЕ УТРО!",
            "сказал всем дратути!", "сказал всем КУ",
            "сказал всем салют!", "сказал всем ЙО!"]
        mass_target = [
            "поприветствовал ", "поздоровался с ",
            "сказал Салам Алейкум ","сказал ДОБРОЕ УТРО ",
            "сказал КУ ", "сказал салют ", "сказал ЙО "]
    ...
    async def letsplay(self, ctx, member: discord.Member, game=None):
        ...
        if game == None:
            await ctx.send(
                f"{ctx.author.mention} зовет играть {member.mention}")
            await ctx.send(file=discord.File(f"./Images/GIFS/Play/{name}"))

        else:
            await ctx.send(
                f"{ctx.author.mention} зовет играть "
                f"{member.mention} в **{game}**")
            await ctx.send(file=discord.File(f"./Images/GIFS/Play/{name}"))
