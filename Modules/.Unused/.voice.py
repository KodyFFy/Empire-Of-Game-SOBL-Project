...
class Voice(BOT.Cog):
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        c.execute("SELECT voiceChannelID FROM guild WHERE guildID = ?",
                  (guildID, ))
        voice = c.fetchone()
        ...
        if voice is None:
            ...
            try:
                if after.channel.id == voiceID:
                    c.execute("SELECT * FROM voiceChannel WHERE userID = ?",
                              (member.id, ))
                    cooldown = c.fetchone()
                    ...
                    if cooldown is None:
                        ...
                        await member.send(
                            "Creating channels too quickly you've been put "
                            "on a 15 second cooldown!")
                    ...
                    await channel2.set_permissions(self.bot.user,
                        connect=True,
                        read_messages=True)
                    await channel2.edit(name=name, user_limit=limit)
                    c.execute("INSERT INTO voiceChannel VALUES (?, ?)",
                              (id, channelID))
                    ...
                    c.execute("DELETE FROM voiceChannel WHERE userID=?",
                              (id, ))
            except:
                pass
    ...
    async def voiceHelp(self, ctx):
        ...
        embed.add_field(
            name="**BOT**",
            value="**Lock your channel by using the following command:**\n\n`.voice lock`\n\n------------\n\n"
    ...
    async def setup(self, ctx):
        ...
        id = ctx.author.id
        if ctx.author.id == ctx.guild.owner.id or ctx.author.id == 561880157611360259:
            ...
            try:
                ...
                new_cat = await ctx.guild.create_category_channel(
                    category.content)
                await ctx.channel.send(
    ...
    async def lock(self, ctx):
        ...
        if voice is None:
            await ctx.channel.send(
                f"{ctx.author.mention} You don't own a channel.")
        else:
            channelID = voice[0]
            ...
            await ctx.channel.send(
                f"{ctx.author.mention} Voice chat locked! 🔒")
        conn.commit()
        conn.close()
    ...
    async def unlock(self, ctx):
        ...
        id = ctx.author.id
        c.execute(
            "SELECT voiceID FROM voiceChannel WHERE userID = ?", (id, ))
        voice = c.fetchone()
        ...
        if voice is None:
            ...
            await ctx.channel.send(
                f"{ctx.author.mention} Voice chat unlocked! 🔓")
        conn.commit()
        conn.close()

    @BOT.command(aliases=["allow"])
    ...
    async def limit(self, ctx, limit):
        ...
        if voice is None:
            ...
            await ctx.channel.send(
                ...
                "{}!".format(limit))
            c.execute("SELECT channelName FROM userSettings WHERE userID = ?",
                      (id, ))
            voice = c.fetchone()
            if voice is None:
    ...
    async def claim(self, ctx):
        ...
        if channel == None:
            await ctx.channel.send(
                f"{ctx.author.mention} you're not in a voice channel.")

        else:
            id = ctx.author.id
            c.execute("SELECT userID FROM voiceChannel WHERE voiceID = ?",
                      (channel.id, ))
            voice = c.fetchone()

            if voice is None:
                await ctx.channel.send(
                    f"{ctx.author.mention} You can't own that channel!")
