...
class RPG(BOT.Cog):
    ...
    async def train(self, ctx, skill):
        ...
        if clas == "DPS":
            ...
            if skill == "DPS" and lvl_skill > maxDPSlvl:
                ...
                await ctx.send(
                    f"Ваш уровень скила {skill} повышен на {lvlup}")
    ...
    async def lvl(slef, ctx):
        ...
        embed=discord.Embed(
            title="**Статистика ваших классов**",
            description=f"Выбранный класс: {clas}",
            color=0x212ee4)
        embed.add_field(name="DPS",
        value=f"Уровень: {int(DPS)};Максиммальный уровень {maxlvls[0]}",
        inline=False)
        embed.add_field(name="TANK",
            value=f"Уровень: {int(TANK)}; Максиммальный уровень {maxlvls[1]}",
            inline=False)
        embed.add_field(name="HEALER",
            value=f"Уровень: {int(HEALER)}; Максиммальный уровень {maxlvls[2]}",
            inline=False)
