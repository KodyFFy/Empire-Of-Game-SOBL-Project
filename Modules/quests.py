        ...
        if int(money) > int(balance):
            ...
            try:
                ...
                if user_answ.content == "start":
                        ...
                        try:
                            ...
                            try:
                                if int(user_answ.content) == int(answ):
                                    kof += 1
                                    new_money = (
                                        float(new_money) * 0.02 * float(kof))
                                    ger = round(int(new_money), 0)
                                    await channel.send(
                                        f"Верно! Идем дальше. Правльных "
                                        f"ответов: {kof}, Ваш выйгрыш: {ger}"
                                    )
                    ...
                    embed = discord.Embed(
                        title="Результаты",
                        description=f"Вы ответили правильно на {kof} "
                                     "примеров.\n Ваша ставка превратилась в "
                                    f"{ger} <:coin:791004475098660904>"
                    )
                    await channel.send(embed=embed)
                    ...
                    await channel.send(
                        ...
                        "сделайте скрин.\nНа все у вас есть 30 секунд. "
                        "После канал **самоуничтожится!**"
                    )
                    ...
                    await channel.send(
                        "Мы вернули вашу ставку!. Через 5 секунд канал "
                        "**самоуничтожится!**"
                    )
                    users[str(user.id)]["Wallet"] += int(money)
